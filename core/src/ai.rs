...
pub struct AiSettingsInfo {
    ...
    pub profiles: Vec<AiProfileInfo>,
}
...
pub async fn get_settings_info() -> AiSettingsInfo {
    ...
    match active {
        Some(settings) => AiSettingsInfo {
            ...
            profiles: profiles.iter().map(ai_profile_info).collect(),
        },
        None => AiSettingsInfo {
...
pub async fn save_settings(
    ...
    base_url: Option<&str>,
    api_key: Option<&str>,
) -> Result<AiProviderSettingsRecord, String> {
    let provider = normalize_provider(provider);
    let model = model.trim();
    let id = normalize_profile_id(id.unwrap_or("default"));
    let name = name
        .map(str::trim)
    ...
    let encrypted_api_key = if provider == "ollama" {
        ...
        match api_key.map(str::trim) {
            Some("") | None => existing
                ...
                .unwrap_or_default(),
            Some(api_key) => {
                encrypt_secret(api_key.as_bytes()).map_err(|err| format!("Failed to encrypt API key: {err}"))?
            }
        }
    };
    ...
    app_db::save_ai_provider_settings(&settings)
        .await
        .map_err(|err| format!("Failed to save AI settings: {err}"))?;
    Ok(settings)
}

pub async fn set_active_profile(id: &str) -> Result<(), String> {
    let id = normalize_profile_id(id);
    if id.is_empty() {
        return Err("AI agent is required.".to_string());
    }
    if app_db::get_ai_provider_settings_by_id(&id).await.is_none() {
        return Err("AI agent was not found.".to_string());
    }
    app_db::set_active_ai_provider_settings(&id)
        .await
        .map_err(|err| format!("Failed to activate AI agent: {err}"))
}
...
fn ai_profile_info(settings: &AiProviderSettingsRecord) -> AiProfileInfo {
    AiProfileInfo {
        id: settings.id.clone(),
        name: ai_profile_name(settings),
        provider: settings.provider.clone(),
...
pub async fn test_connection() -> Result<String, String> {
    let Some(settings) = load_settings_with_key().await else {
        return Err("Save AI provider settings before testing.".to_string());
    };
    let messages = vec![
        json!({"role": "system", "content": "Reply with a short confirmation that the AI provider is reachable."}),
        json!({"role": "user", "content": "Say OK for OG TestDesk."}),
    ];
    call_provider(&settings, messages).await
}

pub async fn list_models() -> Result<(String, Vec<String>), String> {
    let Some(settings) = load_settings_with_key().await else {
        return Err("Save AI provider settings before loading models.".to_string());
    };
    let models = list_provider_models(&settings).await?;
    Ok((settings.provider, models))
}

pub async fn chat(request: AiChatRequest) -> Result<AiChatResponse, String> {
    let Some(settings) = load_settings_with_key().await else {
        return Err("Save AI provider settings before using the assistant.".to_string());
    };

    let user_message = truncate_chars(request.message.trim(), MAX_MESSAGE_CHARS);
    if user_message.is_empty() {
        return Err("Message is required.".to_string());
    }

    let context = request.context.as_ref().map(redact_value).unwrap_or_else(|| json!({}));
    ...
    let page = request.page.as_deref().unwrap_or("unknown");
    let context_summary = truncate_chars(request.context_summary.as_deref().unwrap_or(""), 1200);

    let system_prompt = r#"You are the read-only AI Assistant inside OG TestDesk, a developer tool for SQL, HTTP requests, JSON inspection, and scratch notes.
...
If context contains redacted values, do not ask the user to reveal secrets unless absolutely necessary. Keep answers practical and concise."#;
    ...
    let message = call_provider(&settings, messages).await?;
    Ok(AiChatResponse {
        message,
...
async fn list_provider_models(settings: &RuntimeSettings) -> Result<Vec<String>, String> {
    match settings.provider.as_str() {
        ...
        "anthropic" => {
            Err("Model discovery is not implemented for Anthropic yet. Enter the model manually.".to_string())
        }
        _ => Err("Unsupported AI provider.".to_string()),
    }
}
...
async fn call_openai_compatible(settings: &RuntimeSettings, messages: Vec<Value>) -> Result<String, String> {
...
async fn call_gemini(settings: &RuntimeSettings, messages: Vec<Value>) -> Result<String, String> {
    let base_url = gemini_base_url(settings);
    let endpoint = format!("{base_url}/models/{}:generateContent?key={}", settings.model, settings.api_key);
...
fn normalize_provider(provider: &str) -> String {
    match provider.trim().to_lowercase().as_str() {
        ...
        "custom" | "openai_compatible" | "custom_openai_compatible" => "custom_openai_compatible".to_string(),
...
fn decrypt_secret(encrypted_hex: &str) -> io::Result<String> {
    ...
    String::from_utf8(plaintext).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err.to_string()))
}
